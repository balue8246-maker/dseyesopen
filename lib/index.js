...
export const inject = ['llm', 'tools', 'settings', 'timer']
...
export function apply(ctx, config = {}) {
  ...
  // 识别 content 中（含 tool-result 嵌套）的图片块：顶层 image 或 tool-result.content 里的 image。
  function hasNestedImage(blocks) {
    if (!Array.isArray(blocks)) return false
    return blocks.some((b) => {
      if (!b) return false
      if (b.type === 'image') return true
      if (b.type === 'tool-result' && Array.isArray(b.content)) return hasNestedImage(b.content)
      return false
    })
  }
  ...
  async function transformMessages(messages, signal) {
    ...
    for (const message of messages) {
      ...
      const hasImageBlock = hasNestedImage(content)
      ...
      for (const block of content) {
        ...
        // tool-result 嵌套图片：read_image 等工具返回的图片在这里。
        // 递归替换其中的 image 块为识别文字（保留 tool-result 包装结构）。
        if (block && block.type === 'tool-result' && Array.isArray(block.content) && hasNestedImage(block.content)) {
          const inner = []
          for (const sub of block.content) {
            if (sub && sub.type === 'image') {
              const rec = await recognize(sub.attachment, contextText, signal)
              if (rec.ok) inner.push({ type: 'text', text: '【图片已由识图模型(' + model + ') 自动识别，以下为图片内容】\n' + rec.text })
              else inner.push({ type: 'text', text: failureText(rec.error) })
            } else {
              inner.push(sub)
            }
          }
          blocks.push({ ...block, content: inner })
          continue
        }
        blocks.push(block)
      }
      ...
      if (!reg.adapter[STREAM_WRAP_FLAG]) {
        ...
        if (typeof originalStream === 'function') {
          reg.adapter.stream = async function* streamWithVision(options) {
            ...
            const hasImage = Array.isArray(messages) && messages.some((m) => Array.isArray(m && m.content) && hasNestedImage(m.content))
            if (!hasImage) {
  ...
  // 持续兜底：adapters-updated 可能错过（replace 发生在事件间隙），
  // 周期性检查并重建包装（幂等，包装过则跳过）。30s 间隔足够覆盖 replace 时机。
  const WRAP_WATCH_INTERVAL_MS = 30000
  const watchTimer = typeof ctx.setInterval === 'function'
    ? ctx.setInterval(() => { wrapDeepseekAdapter() }, WRAP_WATCH_INTERVAL_MS)
    : setInterval(() => { wrapDeepseekAdapter() }, WRAP_WATCH_INTERVAL_MS)
  ...
  // 注意：disposer 执行时 settings 服务可能已销毁，mutate 的 Promise 会 reject——
  // 必须给 Promise 挂 catch，否则 unhandled rejection 会被 cordis 判为 fatal load failure。
  ctx.effect(function* () {
    yield () => {
      try {
        const section = ctx.settings && ctx.settings.get(SETTINGS_NS)
        ...
        const pending = ctx.settings.mutate(SETTINGS_NS, [{ op: 'unset', path: ['providers', provider] }])
